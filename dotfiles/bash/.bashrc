...
# Adding sass compiler to the terminal
export PATH="/home/duduya/.builds/sass/dart-sass:$PATH"
#Adding the cl command that cd`s then ls`s
cl() {
	local dir="$1"
	local dir="${dir:=$HOME}"
	if [[ -d "$dir" ]]; then
